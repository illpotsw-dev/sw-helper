...
## 1. One-liner
A web app that helps players in a Discord-based fantasy nation-roleplay game generate and format the four recurring, structured reports a nation has to post each turn: its budget, its Order of Battle, its Navy, and its weapons stockpile.
...
- **Battle Losses** — after a battle is fought narratively in Discord, the player comes back with a total ("6,700 men and eleven guns") and has to turn it into a roster. The app takes the total, the formations that fought, and a few steering controls — per-row weights, exact locks, a jitter setting — and writes the resulting strengths back to the Order of Battle as one undoable step. Run the other way it distributes reinforcements. It never decides *how many* were lost, only *who* took them. Defined in [mvp-battle-losses.md](mvp-battle-losses.md).
...
- **Weapons Stockpile** — every weapon the nation owns that nobody is carrying, accumulated across a campaign by purchase and capture from any number of nations. It is not a standalone quantity list: the stockpile and the Order of Battle are two halves of one closed ledger, so a weapon is either in a unit's hands or in the pile, and re-arming a brigade returns the old pattern and draws the new one in a single undoable movement. Unlimited capacity, no upkeep. Defined in [mvp-stockpile.md](mvp-stockpile.md); narrowed from "equipment" to weapons there, and why.
- **Turn Budget** — per-turn income and expense line items. Upkeep costs are pulled automatically from the current Order of Battle and Navy rather than re-entered (the Weapons Stockpile carries no upkeep), and the app computes the net (income − expenses − upkeep) for a formatted budget report each turn. Export format defined in [mvp-budget.md](mvp-budget.md).
- **Starting Data Templates** — editable template files (YAML) define a new nation's initial values: starting army units and their upkeep, starting ships and their upkeep, the nation's weapon catalog and starting stockpile, and starting treasury/budget baseline. There are two distinct ways this data gets into the app:
...
2. **Post a turn budget:** Open Turn Budget → app pulls current upkeep from Order of Battle/Navy → player adds this turn's income/expense line items → preview computed report (income, expenses, upkeep, net) → copy/export → paste into Discord.
3. **Update a tracker:** After an in-game event (e.g. a unit is destroyed, a ship is built, stockpile is consumed), player edits the relevant tracker (Order of Battle / Navy / Weapons Stockpile) → change is written to the local SQLite DB and reflected in the next budget's upkeep and any re-exported report.
