...
- **Unit instance** — an actual raised unit attached to a formation: its own designation (e.g. "II/I Levy Battalion"), a reference to a unit *type* from the nation's [`land-units.yml`](../nations/template/land-units.yml) catalog (description, recruit cost, upkeep, build time already defined there), its *current* men/weapons strength, and what it is actually armed with. Strength and arming live on the instance rather than the type because units take losses between turns and because two units of the same type are frequently equipped differently (one battalion re-armed with a newer rifle while its sisters keep the old one). Arming is a **holding** — a weapon from the nation's `weapons.yml` catalog plus a count of them, never more than the unit's strength — rather than a free-text label, because the stockpile and the OOB are two halves of one ledger; see [mvp-stockpile.md](mvp-stockpile.md) §2. A unit instance may sit at **zero strength** — a paper unit, on the books with nobody in it — so a battalion wiped out in battle keeps its designation, its weapon assignment and its place in the tree rather than being deleted, though the weapons themselves are gone; see [mvp-battle-losses.md](mvp-battle-losses.md) §2.1. Upkeep scales with how much of its type's establishment a unit actually fields, so a paper unit costs nothing and a half-strength battalion costs half.
...
## 4. Data File Format
The tree persists/imports as YAML — see [`mvp/nations/template/army-oob.yml`](../nations/template/army-oob.yml). It's two flat lists rather than nested YAML: a `formations` list where each entry links to its parent by `id` (deep hand-edited nesting is too easy to indent wrong), and a `units` list where each entry attaches one named unit — type, current strength, `weapon` and optional `weapon_count` ([mvp-stockpile.md](mvp-stockpile.md) §7) — to a `formation_id`. Clan McGreggor's filled-in version is at [`mvp/nations/clan-mcgreggor/army-oob.yml`](../nations/clan-mcgreggor/army-oob.yml). This is the same paste-based import/pre-defined-nation-file split as the other trackers (see mvp.md Section 5, Starting Data Templates).
